...
public class IntergrationTest extends BaseTest {
    ...
    @Owner("Varthika")
    @Description("Verify user able to update the Booking id")
    public void UpdateBookingIS() throws JsonProcessingException {
        requestSpecification.basePath(APIConstants.UPDATE_BOOKING+"/9351");

        Response response = RestAssured.given().spec(requestSpecification)
                .when().body(payloadManager.createPayload()).put();
        ValidatableResponse validatableResponse = response.then().log().all();
        jsonPath = JsonPath.from(response.asString());
